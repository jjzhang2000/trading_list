...
基于技术指标的上证A股筛选系统，通过多维度技术指标筛选多头趋势股票，并按趋势强度排序。GUI界面将股票（60开头，扣除持仓）和ETF（5开头，扣除持仓）分别筛选，持仓与已平仓股票直接计算指标，结果分五个tab展示：股票、ETF、持仓、历史、查询（手动输入代码查看指标值）。
...
2. **数据库**：SQLite，5年历史数据
3. **提取性能**：数据提取使用并发下载（默认3线程），日常增量更新只拉取最近50条数据判断后增量写入
