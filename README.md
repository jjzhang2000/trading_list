...
本项目是一个基于技术指标的证券筛选系统，主要用于分析上证A股（60开头）和上证ETF（5开头），通过多维度技术指标筛选出具有多头趋势的证券，并根据趋势强度进行评分排序。GUI界面将结果分四个tab展示：股票（60开头，扣除持仓）、ETF（5开头，扣除持仓）、持仓（直接计算指标，不筛选）、历史（已平仓股票，扣除持仓）。
...
- **持仓股票管理**：从数据库交易流水自动获取持仓股票（净持仓>0）；GUI中结果分四个tab展示（股票、ETF、持仓、历史）
