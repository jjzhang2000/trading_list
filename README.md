# 股票筛选系统

## 项目概述

本项目是一个基于技术指标的证券筛选系统，主要用于分析上证A股（60开头）和上证ETF（5开头），通过多维度技术指标筛选出具有多头趋势的证券，并根据趋势强度进行评分排序。GUI界面将结果分五个tab展示：股票（60开头，扣除持仓）、ETF（5开头，扣除持仓）、持仓（直接计算指标，不筛选）、历史（已平仓股票，扣除持仓）、查询（手动输入代码查看指标值与总分）。

## 核心功能

- **多指标筛选**：SuperTrend、Vegas通道、布林带、OCC、VP Slope 五个技术指标
- **5指标加总评分**：SuperTrend、Vegas、布林带、OCC、VP Slope 五个指标简单求和作为综合强度评分
- **趋势强度评分**：综合评分系统，量化多头趋势强弱
- **持仓股票管理**：从数据库交易流水自动获取持仓股票（净持仓>0）；GUI中结果分五个tab展示（股票、ETF、持仓、历史、查询）
- **手动查询**：在筛选器组的查询框输入股票代码，实时计算并展示该股票的指标值与总分到「查询」tab，未找到时给出提示
- **总分升降着色**：各tab（股票/ETF/持仓/历史/查询）中的每只股票，按最近两个交易日总分升降设置字体颜色——总分上升为绿色、下降为红色、不变为黑色
- **K线图查看**：双击股票/ETF打开独立窗口，使用lightweight-charts展示K线图和成交量，可叠加显示Vegas EMA通道、布林带，并随鼠标显示布林带开口率与VP Slope斜率
- **总分趋势图**：筛选结果下方内置图表框，选中任一tab（股票/ETF/持仓/历史/查询）中的股票时，自动绘制该股票近30个交易日的指标总分变化曲线（按自然周划分，每周一显示日期和竖向网格线）
- **窗口状态记忆**：GUI 和 K 线窗口关闭时自动保存位置与尺寸；K线窗口额外记忆指标开关与日期范围，下次启动自动恢复
- **ST股票过滤**：自动过滤名称含ST的股票（ETF自动跳过ST检查）
- **交易流水迁移**：从Excel文件迁移两个账户的交易流水到数据库（含去重与证券代码补全）
- **交易盈亏统计**：GUI交易组内统计各账户的银行转入/转出、净投入、已平仓盈亏（含股息、红利等资金类收支），以及当前持仓的估值与浮盈（按最近交易日收盘价）
- **右键临时买卖**：持仓tab右键"卖出"（按各账户持仓数）、股票/ETF/历史tab右键"买入"（固定100股），按数据库最新收盘价记录临时交易流水（业务名带"临时"标记）；迁移正式流水时自动删除全部临时记录
- **双界面支持**：命令行(CLI)和图形界面(GUI)

## 项目结构

```
trading_list/
├── trading_list.py       # 股票筛选主程序（命令行）
├── list_gui.py           # 股票筛选主程序（图形界面）
├── kline_window.py       # K线图窗口模块（使用pywebview）
├── kline_standalone.py   # K线图独立窗口脚本
├── requirements.txt      # 依赖项文件
├── .gitignore            # Git忽略文件
├── tech/                 # 技术指标模块目录
│   ├── __init__.py       # 模块初始化文件
│   ├── supertrend.py     # SuperTrend指标计算模块
│   ├── vegas.py          # Vegas通道指标计算模块
│   ├── bollingerband.py  # 布林带指标计算模块
│   ├── occross.py        # Open/Close Cross指标计算模块
│   ├── vp_slope.py       # VolumeProfile Slope指标计算模块
│   └── trend_score.py    # 综合趋势强度评分模块
├── data/
│   ├── __init__.py       # 模块初始化文件
│   ├── extract_data.py   # 前复权价格数据获取脚本
│   ├── init_db.py        # 数据库初始化脚本
│   ├── read_data.py      # 数据库读取模块
│   ├── batch_fetch.py    # 批量数据获取模块
│   ├── migrate_trades.py # 交易流水迁移与盈亏统计模块
│   └── stock_data.db     # 股票数据库
└── utils/
    ├── __init__.py       # 模块初始化文件
    ├── logger.py         # 日志模块
    └── window_state.py   # 窗口位置/尺寸及UI状态持久化模块
```

## 快速开始

### 1. 安装依赖

```bash
pip install -r requirements.txt
```

### 2. 运行图形界面（推荐）

```bash
python list_gui.py
```

### 3. 运行命令行版本

```bash
# 默认跳过数据更新，直接筛选
python trading_list.py

# 更新数据后筛选
python trading_list.py --update

# 指定日期筛选
python trading_list.py -d 2025-03-07

# 完整参数
python trading_list.py -d 2025-03-07 -b 10.0 --update
```

## 筛选流程

```
全部股票
    ↓
按代码前缀分组：60开头 → 股票组，5开头 → ETF组
    ↓
=== 股票筛选（60开头，扣除持仓）===
    ↓
过滤ST股票（名称含ST）
    ↓
[1/5] SuperTrend筛选 → 多头趋势
    ↓
[2/5] Vegas通道筛选 → EMA多头排列且连续多头>=5天
    ↓
[3/5] 布林带筛选 → 开口率>阈值 且 收盘价>中轨
    ↓
[4/5] OCC指标筛选 → 多头趋势
    ↓
[5/5] VP Slope筛选 → 长期斜率 slope_long > 0
    ↓
计算趋势强度评分（截面Z-score）
    ↓
结果填入「股票」tab
    ↓
=== ETF筛选（5开头，扣除持仓）===
    ↓
ETF跳过ST检查
    ↓
[1/5] SuperTrend筛选 → 多头趋势
    ↓
[2/5] Vegas通道筛选 → EMA多头排列且连续多头>=5天
    ↓
[3/5] 布林带筛选 → 开口率>阈值 且 收盘价>中轨
    ↓
[4/5] OCC指标筛选 → 多头趋势
    ↓
[5/5] VP Slope筛选 → 长期斜率 slope_long > 0
    ↓
计算趋势强度评分（截面Z-score）
    ↓
结果填入「ETF」tab
    ↓
=== 持仓计算（不筛选，直接计算指标）===
    ↓
从数据库交易流水读取持仓列表（净持仓>0，仅统计证券买入/卖出/红股入账）
    ↓
对持仓股票直接计算所有指标（SuperTrend、Vegas、布林带、OCC、VP Slope）
    ↓
计算趋势强度评分
    ↓
结果填入「持仓」tab
    ↓
=== 历史（已平仓）计算（不筛选，直接计算指标）===
    ↓
从数据库交易流水读取已平仓列表（净持仓=0，扣除持仓）
    ↓
对已平仓股票直接计算所有指标
    ↓
计算趋势强度评分
    ↓
结果填入「历史」tab
```

## 技术指标详解

### 1. SuperTrend

趋势跟踪指标，基于ATR计算支撑/阻力位。

| 参数    | 默认值 |
| ----- | --- |
| ATR周期 | 10  |
| ATR乘数 | 3.0 |

**判断规则**：

- trend_direction = 1: 多头
- trend_direction = -1: 空头

### 2. Vegas通道

由3条EMA组成的多头排列通道系统。

| 通道类型 | EMA周期 |
| ---- | ----- |
| 短期   | 12   |
| 中期   | 144  |
| 长期   | 576  |

**多头排列条件**：

```
close > EMA12 > EMA144 > EMA576
```

**筛选要求**：多头排列需**持续 >= 5个交易日**，仅当日多头不足。

**数据不足退化**：当历史数据不足576条、EMA576无法计算时，退化为仅判断 `close > EMA12 > EMA144`。

### 3. 布林带

波动率指标，由中轨和上下轨组成。

| 参数    | 默认值 |
| ----- | --- |
| 周期    | 20  |
| 标准差倍数 | 2.0 |

**筛选条件**：

- 开口率 > 阈值（默认10%）
- 收盘价 > 中轨

**开口率计算**：

```
开口率 = (上轨 - 下轨) / 中轨 × 100%
```

### 4. OCC (Open/Close Cross)

通过开盘价和收盘价的移动平均交叉判断趋势。

| 参数   | 默认值         |
| ---- | ----------- |
| 周期   | 8           |
| MA类型 | TMA（三角移动平均） |

**支持的MA类型**：SMA, EMA, DEMA, TEMA, WMA, VWMA, SSMA, TMA

**判断规则**：

- occ_close > occ_open: 多头
- occ_close < occ_open: 空头

### 5. VP Slope

通过线性回归计算价格趋势斜率。

| 参数   | 默认值 |
| ---- | --- |
| 长期周期 | 100 |
| 短期周期 | 10  |

**判断规则**：

- slope_long > 0: 上升趋势
- slope_long <= 0: 下降/走平

## 综合评分（5指标加总）

### 评分方式

从 `stock_indicators` 表读取 5 个指标值（每个值均已被 clamp 到 [-100, 100]），简单求和即为综合评分：

```
strength_score = supertrend + vegas + bollingerbands + openclosecross + volumeprofile
```

| 指标            | 含义                          | 取值范围    |
| --------------- | ----------------------------- | ---------- |
| supertrend      | 收盘价高于SuperTrend线的幅度(%) | -100 ~ 100 |
| vegas           | 收盘价高于EMA144的幅度(%)       | -100 ~ 100 |
| bollingerbands  | 布林带开口率(%)                | -100 ~ 100 |
| openclosecross  | OCC偏离度(×1000)              | -100 ~ 100 |
| volumeprofile   | 短期斜率占比(×1000)            | -100 ~ 100 |

缺失的指标按 0 计入，总分范围约 -500 ~ 500，按总分降序排名。

## 输出结果

### CSV文件

文件名格式：`listing-YYYY-MM-DD_HHMMSS.csv`

输出列：

| 列名              | 说明                          |
| --------------- | ----------------------------- |
| rank            | 排名                          |
| stock_code      | 股票代码                      |
| stock_name      | 股票名称（持仓股名前加 `*`）    |
| strength_score  | 综合强度评分（5指标总和）       |
| supertrend      | SuperTrend偏离度              |
| vegas           | 收盘价高于EMA144的幅度(%)       |
| bollingerbands  | 布林带开口率(%)                |
| openclosecross  | OCC偏离度(×1000)              |
| volumeprofile   | 短期斜率占比(×1000)            |

### 示例输出

```csv
rank,stock_code,stock_name,strength_score,supertrend,vegas,bollingerbands,openclosecross,volumeprofile
1,605376,博迁新材,215,34,55,28,45,53
2,603256,宏和科技,178,41,32,19,38,48
```

## 命令行参数

| 参数                | 说明       | 默认值  |
| ----------------- | -------- | ---- |
| `-d, --date`      | 筛选日期     | 今天   |
| `-b, --bandwidth` | 布林带开口率阈值 | 10.0 |
| `-p, --proxy`     | 代理服务器地址  | 无    |
| `--update`        | 更新股票数据   | 跳过   |

## 数据来源

- **股票列表**：新浪财经API（上证A股60开头股票 + 上证ETF 5开头）
- **历史K线数据**：新浪财经API
- **复权因子**：新浪财经API
- **前复权价格计算**：前复权价格 = 原始价格 / 后复权因子

## 注意事项

1. **数据完整性**：确保网络连接正常，以便获取最新的股票数据
2. **API限制**：数据提取采用并发下载（默认3线程，兼顾速度与接口频率限制），日常增量更新只拉取最近少量数据，避免全量重复下载
3. **复权因子更新**：系统会自动检测复权因子变动并重新下载数据
4. **EMA收敛**：Vegas通道的EMA576需要足够历史数据才能收敛；当数据不足576条、EMA576无法计算时，自动退化为仅判断 `close > EMA12 > EMA144` 的两线多头排列
5. **截面排序**：Z-score基于当日筛选池股票计算，评分具有相对性
6. **交易盈亏统计的已知限制**：交易流水中的「股份转出」「配股权证」等拆股/转股类业务会改变持仓股数，但发生金额为 0。当前盈亏统计（`data/migrate_trades.py` 的 `get_trade_pnl_stats`）尚未处理这类股数变化，它们暂不影响盈亏数值。等未来出现这类业务实际影响盈亏（例如拆股后卖出）时，再补充相应的股数增减逻辑。

## 扩展方向

- 添加更多技术指标
- 实现更复杂的评分算法
- 添加数据可视化功能
- 实现自动交易策略
- 添加回测功能
- 添加风险管理模块

## 许可证

本项目仅供学习和研究使用，不得用于商业用途。
