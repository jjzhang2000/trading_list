...
本项目是一个基于技术指标的证券筛选系统，主要用于分析上证A股（60开头）和上证ETF（51开头），通过多维度技术指标筛选出具有多头趋势的证券，并根据趋势强度进行评分排序。
...
- **持仓股票管理**：自动加入持仓股票；GUI中持仓股票与筛选结果分两个等高表格展示
...
- **股票列表**：新浪财经API（上证A股60开头股票 + 上证ETF 51开头）
- **历史K线数据**：新浪财经API
- **复权因子**：新浪财经API
