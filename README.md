...
本项目是一个基于技术指标的证券筛选系统，主要用于分析上证A股（60开头）和上证ETF（5开头），通过多维度技术指标筛选出具有多头趋势的证券，并根据趋势强度进行评分排序。GUI界面将结果分三个tab展示：股票（60开头，扣除持仓）、ETF（5开头，扣除持仓）、持仓（直接计算指标，不筛选）。
...
- **K线图查看**：双击股票/ETF打开独立窗口，使用lightweight-charts展示K线图和成交量，可叠加显示Vegas EMA通道、布林带，并随鼠标显示布林带开口率与VP Slope斜率
- **窗口状态记忆**：GUI 和 K 线窗口关闭时自动保存位置与尺寸；K线窗口额外记忆指标开关与日期范围，下次启动自动恢复
- **ST股票过滤**：自动过滤名称含ST的股票（ETF自动跳过ST检查）
- **交易流水迁移**：从Excel文件迁移两个账户的交易流水到数据库（含去重与证券代码补全）
- **交易盈亏统计**：GUI交易组内统计各账户的银行转入/转出、净投入，以及已平仓盈亏（含股息、红利等资金类收支）
...
│   ├── migrate_trades.py # 交易流水迁移与盈亏统计模块
│   └── stock_data.db     # 股票数据库
└── utils/
    ├── __init__.py       # 模块初始化文件
    ├── logger.py         # 日志模块
    └── window_state.py   # 窗口位置/尺寸及UI状态持久化模块
```
...
=== 持仓计算（不筛选，直接计算指标）===
    ↓
读取持仓列表（shareholding.txt）
    ↓
对持仓股票直接计算所有指标（SuperTrend、Vegas、布林带、OCC、VP Slope）
    ↓
计算趋势强度评分
    ↓
结果填入「持仓」tab
...
- **股票列表**：新浪财经API（上证A股60开头股票 + 上证ETF 5开头）
- **历史K线数据**：新浪财经API
- **复权因子**：新浪财经API
